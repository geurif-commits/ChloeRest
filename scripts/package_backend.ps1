...
if (Test-Path "ServidorPOS.exe") {
  $filesToZip = @("ServidorPOS.exe")
  if (Test-Path "$repoRoot\start_servidorpos.bat") {
    $filesToZip += "$repoRoot\start_servidorpos.bat"
  }
  if (Test-Path "$repoRoot\.env.example") {
    $filesToZip += "$repoRoot\.env.example"
  }
  Write-Output "[package_backend] Comprimiendo artefactos backend -> $Output"
  if (Test-Path $Output) { Remove-Item $Output -Force }
  Compress-Archive -Path $filesToZip -DestinationPath $Output -Force
