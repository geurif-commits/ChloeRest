...
export default function LogoFondoSettings({ apiUrl }) {
  ...
  const guardar = async (e) => {
    ...
    try {
      ...
      if (res.ok) { toastAviso('✅ Logo y fondo guardados correctamente.'); }
  ...
  if (cargando) return <p style={{ color: 'var(--text-secondary)', padding: 20 }}>Cargando configuración...</p>;
    ...
    <div className="logo-fondo-settings">
      <div className="logo-fondo-settings__form-col">
        <form onSubmit={guardar} className="logo-fondo-form">
          <h3 className="logo-fondo-form__title">🖼️ Logo del Negocio</h3>
          ...
          <h3 className="logo-fondo-form__title">🌄 Imagen de Fondo (Pantalla de PIN)</h3>
          <div className="logo-fondo-form__specs">
            ...
            <ul>
              ...
              <li><strong>Dimensiones recomendadas:</strong> 1920×1080 px (16:9) o superior</li>
              <li><strong>Resolución mínima:</strong> 1280×720 px para evitar pixelación</li>
              <li><strong>Contenido recomendado:</strong> Imágenes de paisajes, texturas o abstractas con pocos detalles. Evitar texto o logos en la imagen de fondo ya que se superpondrá con el contenido del login.</li>
              <li><strong>Nota:</strong> La imagen se muestra con modo "cover" (cubre toda la pantalla) y se oscurece automáticamente para mejorar la legibilidad del PIN.</li>
            </ul>
          </div>
          ...
          <button type="submit" disabled={guardando} className="tema-save-btn">
            {guardando ? 'Guardando...' : '💾 Guardar Logo y Fondo'}
          </button>
        </form>
      </div>

      <div className="logo-fondo-settings__preview-col">
        <h3 className="logo-fondo-form__title">👁️ Vista Previa (Pantalla de PIN)</h3>
        ...
        <p className="logo-fondo-preview__note">Así se verá la pantalla de PIN con la imagen de fondo, el logo y la opacidad configurada.</p>
      </div>
    </div>
  );
}
