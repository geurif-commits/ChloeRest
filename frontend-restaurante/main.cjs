...
function findBackendLauncher() {
  ...
  const candidates = [
    ...
    path.resolve(baseDir, '..', 'ServidorPOS.exe'),
    path.resolve(baseDir, '..', 'resources', 'ServidorPOS.exe'),
    path.resolve(process.resourcesPath || baseDir, 'ServidorPOS.exe'),
    path.resolve(process.resourcesPath || baseDir, '..', 'ServidorPOS.exe'),
    path.resolve(process.resourcesPath || baseDir, '..', 'resources', 'ServidorPOS.exe'),
  ...
  for (const candidate of candidates) {
    if (fs.existsSync(candidate)) {
      ...
      return { command: 'node', args: [candidate], cwd: path.dirname(candidate) };
    }
  }
  return null;
}
...
async function startBackendIfNeeded() {
  ...
  console.log(`Iniciando backend POS con ${launcher.command} ${launcher.args.join(' ')}`);
