...
| B2 / H2 | Sin respaldos automáticos | ✅ Resuelto en código | Respaldo diario verificado, restauración probada (`npm run backup:verify`), API + pestaña *Respaldos*, activado por defecto en Electron. `BACKUP_COPY_DIR` copia cada respaldo verificado a una segunda carpeta (USB, nube sincronizada, red). **Falta:** definirla en cada instalación y crear el rol con `BYPASSRLS` en el servidor central |
