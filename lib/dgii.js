...
// insumo no tiene costo cargado, cae a 50.00 (valor por defecto documentado en
// el runbook de certificación DGII) para no subreportar movimientos.
export function formatearFila606(g) {
  const fechaComp = fechaDGII(g.fecha);
  const costo = Number(g.costo_unitario) > 0 ? Number(g.costo_unitario) : 50.00;
