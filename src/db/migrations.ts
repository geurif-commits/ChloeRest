/**
 * @file Migraciones de esquema PostgreSQL + arranque de base de datos.
 * Puerto directo de migrations.js (y del fix de consistencia de server.js) a TypeScript.
 *
 * NOTA (instalaciones desde BD vacía): las migraciones 001+ esperan las tablas
 * esenciales ya creadas. Para levantar una BD desde cero aplicar primero
 * src/db/schema_base.sql (recuperado de la historia git) y luego runMigrations().
 */
...
export async function runMigrations(pool: Database): Promise<void> {
  ...
  try {
    ...
    const legacyUsers = await client.query<{ id: number; pin: string }>(`SELECT id, pin FROM usuarios WHERE pin_hash IS NULL AND pin IS NOT NULL AND pin <> '' AND estado = 'Activo'`);
    for (const user of legacyUsers.rows) {
      await client.query('UPDATE usuarios SET pin_hash = $1, pin = NULL WHERE id = $2', [hashPin(user.pin), user.id]);
    }

    // Solo crear administrador inicial si NO existe ningún usuario activo en el sistema
    const users = await client.query<{ total: number }>("SELECT COUNT(*)::int AS total FROM usuarios WHERE estado = 'Activo'");
    ...
    if (config.ownerPin) {
      const owner = await client.query<{ owner_pin_hash: string | null }>('SELECT owner_pin_hash FROM configuracion_sistema WHERE id = 1');
...
/**
 * Fix one-time de consistencia de BD (puerto directo de server.js): garantiza
 * que exista la empresa 1 y repara filas con empresa_id inválido/null antes de
 * correr las migraciones. Se ejecuta en cada arranque como en el legacy.
 */
export async function fixDatabaseConsistency(db: Database): Promise<void> {
  try {
    ...
    try {
      ...
      `);
      logger.info({ action: 'DB_CONSISTENCY_FIX', details: 'Database consistency fix applied' });
    } finally {
      client.release();
    }
  } catch (err) {
    logger.warn({
      action: 'DB_CONSISTENCY_FIX_SKIPPED',
