...
function WizardSetup({ apiUrl, config, configRegistro, alCompletado }) {
  ...
  // Limpia el temporizador de aplicaciÃ³n del tema al desmontar
  useEffect(() => () => {
  ...
  const siguiente = () => {
    ...
    if (paso === 3 && necesitaAdmin) {
      ...
      if (!/^[0-9]{4,12}$/.test(adminPin)) return setError('El PIN debe contener entre 4 y 12 dÃ­gitos.');
    }
    setPaso((p) => p + 1);
  };
  ...
  const terminar = async () => {
    ...
    try {
      ...
      if (!res.ok) return setError(data.error || 'Error al finalizar la configuraciÃ³n.');
      localStorage.removeItem('pos_theme');
      if (alCompletado) alCompletado(data);
    } catch (e) {
      setError('No se pudo conectar con el servidor. Verifica la red e intÃ©ntalo de nuevo.');
    } finally {
      setGuardando(false);
    }
  };
    ...
    <div style={contenedorEstilo} onLoad={aplicarVista}>
      ...
      <div style={panelEstilo}>
        ...
        {paso === 0 && (
          <>
            <div style={{ textAlign: 'center', marginBottom: '20px' }}>
              ...
              <h1 style={{ margin: 0, fontSize: '1.7rem', fontWeight: 800 }}>ðŸš€ Â¡Bienvenido a ChloeRestaurant!</h1>
            ...
            <button onClick={siguiente} style={{ ...btnEstilo, width: '100%' }}>Comenzar configuraciÃ³n â†’</button>
          </>
        )}

        {pasoVisible === 1 && (
          <>
            <h2 style={{ margin: '0 0 6px 0', fontSize: '1.3rem' }}>ðŸ¢ Datos del Negocio</h2>
            <p style={{ color: 'rgba(255,255,255,0.6)', marginBottom: '20px', fontSize: '0.9rem' }}>Este nombre aparecerÃ¡ en la pantalla de ingreso de PIN.</p>
            ...
            <button onClick={siguiente} style={{ ...btnEstilo, width: '100%' }}>Continuar â†’</button>
          </>
        )}

        {pasoVisible === 2 && (
          <>
            <h2 style={{ margin: '0 0 6px 0', fontSize: '1.3rem' }}>ðŸŽ¨ Apariencia del Sistema</h2>
        ...
        {pasoVisible === 3 && necesitaAdmin && (
          <>
            <h2 style={{ margin: '0 0 6px 0', fontSize: '1.3rem' }}>ðŸ” Cuenta de Administrador</h2>
            <p style={{ color: 'rgba(255,255,255,0.6)', marginBottom: '20px', fontSize: '0.9rem' }}>Crea el acceso principal del dueÃ±o del negocio. Guarda bien este PIN.</p>
            <div style={{ marginBottom: '16px' }}>
              ...
              <input style={inputEstilo} value={adminNombre} onChange={(e) => setAdminNombre(e.target.value)} placeholder="Ej: Juan PÃ©rez" />
            </div>
            <div style={{ marginBottom: '16px' }}>
              <label style={{ display: 'block', marginBottom: '6px', color: 'rgba(255,255,255,0.8)', fontSize: '0.85rem', fontWeight: 600 }}>PIN de Acceso (4 a 12 dÃ­gitos)</label>
        ...
        {paso === totalPasos - 1 && (
          <>
            <div style={{ textAlign: 'center', marginBottom: '18px' }}>
              <div style={{ fontSize: '2.6rem' }}>ðŸŽ‰</div>
              <h2 style={{ margin: '10px 0 6px 0', fontSize: '1.4rem' }}>Â¡Todo listo!</h2>
              <p style={{ color: 'rgba(255,255,255,0.7)', lineHeight: 1.5 }}>
                Tu sistema quedÃ³ personalizado{necesitaAdmin ? ' y la cuenta de administrador fue creada' : ''}. Ya puedes ingresar tu PIN para comenzar.
              </p>
            </div>
            <button onClick={terminar} disabled={guardando} style={{ ...btnEstilo, width: '100%' }}>
              {guardando ? 'Configurando...' : 'Finalizar y entrar al sistema âœ“'}
            </button>
          </>
        )}

        {error && <p style={{ color: '#ff6b6b', marginTop: '14px', textAlign: 'center', fontSize: '0.9rem', fontWeight: 600 }}>âš ï¸ {error}</p>}

        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '18px' }}>
          {paso > 0 && paso < totalPasos - 1 && (
            <button onClick={() => { setError(''); setPaso((p) => p - 1); }} style={{ background: 'transparent', border: '1px solid rgba(255,255,255,0.25)', color: '#fff', padding: '10px 18px', borderRadius: '10px', cursor: 'pointer', fontSize: '0.85rem' }}>â† AtrÃ¡s</button>
          )}
        </div>
      </div>
    </div>
