...
function PanelDueno({ apiUrl, config, alVolver }) {
  ...
  const cargarTodo = async (tok) => {
    ...
    try {
      const [resR, resP, resS, resM] = await Promise.all([
      ...
      const [dR, dP, dS, dM] = await Promise.all([resR.json(), resP.json(), resS.json(), resM.json()]);
      setResumen(dR);
      setPlanes(dP.planes || []);
                ...
                <table className="admin-tabla">
                  ...
                  <tbody>
                    {planes.map((p) => (
                      <tr key={p.id}>
                        <td>
                          <input className="form-input" style={inputStyle} value={p.nombre} onChange={(e) => editarPlan(p, { nombre: e.target.value })} />
                        </td>
                        <td>
                          <select className="form-input" style={inputStyle} value={p.duracion_codigo} onChange={(e) => editarPlan(p, { duracion_codigo: e.target.value })}>
                            {DURACIONES.map((d) => <option key={d.codigo} value={d.codigo}>{d.etiqueta}</option>)}
                          </select>
                        </td>
                        <td>
                          <input className="form-input" style={{ ...inputStyle, width: '90px' }} type="number" min="0" value={p.precio} onChange={(e) => editarPlan(p, { precio: Number(e.target.value) })} />
                        </td>
                        <td>
                          <select className="form-input" style={{ ...inputStyle, width: '90px' }} value={p.moneda} onChange={(e) => editarPlan(p, { moneda: e.target.value })}>
                            <option>RD$</option><option>US$</option><option>€</option>
                          </select>
                        </td>
                        <td style={{ textAlign: 'center' }}>
                          <input type="checkbox" checked={Boolean(p.destacado)} onChange={(e) => editarPlan(p, { destacado: e.target.checked })} />
                        </td>
                        <td style={{ textAlign: 'center' }}>
                          <input type="checkbox" checked={Boolean(p.activo)} onChange={(e) => editarPlan(p, { activo: e.target.checked })} />
                        </td>
                        <td>
                          <button className="btn-accion delete" onClick={() => eliminarPlan(p)} title="Eliminar plan">🗑️</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
                    ...
                    {solicitudes.map((s) => (
                      <tr key={s.id}>
                        ...
                        <td>
                          {s.estado === 'Pendiente' ? (
                            <>
                              <button className="btn-accion" onClick={() => cambiarSolicitud(s.id, 'Pagada')}>✅ Pagada</button>
                              <button className="btn-accion" onClick={() => cambiarSolicitud(s.id, 'Atendida')}>Atender</button>
                              <button className="btn-accion delete" onClick={() => cambiarSolicitud(s.id, 'Rechazada')}>Rechazar</button>
                            </>
                          ) : (
                            <button className="btn-accion" onClick={() => cambiarSolicitud(s.id, 'Pendiente')}>Reabrir</button>
                          )}
                        </td>
                      </tr>
                    ))}
