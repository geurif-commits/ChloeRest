...
const TEMAS = [
  ...
  { id: 'amatista', name: 'Amatista', color: '#a855f7', desc: 'Oscuro con tonos pÃºrpura' },
...
export default function TemaSettings({ apiUrl }) {
  ...
  return (
    <form onSubmit={guardar} className="tema-settings">
      ...
      <div className="tema-grid">
        {TEMAS.map((tema) => (
          <button type="button" key={tema.id} className={`tema-card ${config.tema_activo === tema.id ? 'activo' : ''}`} onClick={() => seleccionarTema(tema.id)}>
            ...
            {config.tema_activo === tema.id && <span className="tema-card__check">âœ“</span>}
          </button>
        ))}
      </div>
      ...
      <h3 className="tema-settings__subtitle">🖌️ Estilo de Pantalla de PIN</h3>
